...
public class GameEntryTypeResource {

	@Inject
	private GameEntryTypeDao dao = null;

    @Inject
    private GameService service = null;

	public GameEntryTypeResource() {
	}


	@GET
	public List<GameEntryType> list() {
		return dao.findAll();
	}
    ...
    @POST
    public Response create(@Auth User user, GameEntryType entity) {
        return ResourceHelper.create(user, entity, service, this.getClass());
    }

    @PUT
    @Path("{id}")
    public Response update(@Auth User user, @PathParam("id") String id, GameEntryType entity) {
