...
import org.oregami.service.ReleaseGroupReasonService;
...
public class ReleaseGroupReasonResource {

	@Inject
	private ReleaseGroupReasonDao dao = null;

	public ReleaseGroupReasonResource() {
	}

    @Inject
    ReleaseGroupReasonService service = null;

	@GET
	public List<ReleaseGroupReason> list() {
		return dao.findAll();
	}
    ...
    @POST
    public Response create(@Auth User user, ReleaseGroupReason entity) {
        return ResourceHelper.create(user, entity, service, this.getClass());
    }

    @PUT
    @Path("{id}")
    public Response update(@Auth User user, @PathParam("id") String id, ReleaseGroupReason entity) {
